-- App admins (built-in founder email + app_super_admins) skip course and
-- lecture-recording insert caps so Stripe Premium quotas do not apply.

create or replace function public.is_plan_cap_exempt(p_user_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select
    (
      ...
      and exists (
        select 1 from public.app_super_admins a where a.user_id = p_user_id
      )
    )
    or exists (
      select 1
      ...
      where u.id = p_user_id
        and lower(coalesce(u.email, '')) = 'raphaelxseo@gmail.com'
    );
$$;

comment on function public.is_plan_cap_exempt(uuid) is
  'True when this user is an app_super_admin or the built-in app-admin email.';
...
comment on function public.enforce_course_creation_cap() is
  'Blocks course inserts past Free(1)/Student(2)/Advanced(5); Premium and app admins unlimited.';
comment on function public.enforce_lecture_recording_cap() is
  'Blocks new live lecture sessions past plan caps per billing period; app admins unlimited.';
