  ...
  try {
    ...
    if (!normalizeDeepgramKey(process.env.DEEPGRAM_API_KEY)) {
      return NextResponse.json(
        {
          error:
            "Live transcription is not configured (missing DEEPGRAM_API_KEY).",
        },
        { status: 503 }
      );
    }
    ...
    void report("live-notes.deepgram_token_exception", e, {});
    return NextResponse.json(
      {
        error:
          e instanceof Error
            ? e.message
            : "Could not mint a live transcription token.",
      },
      { status: 500 }
    );
  }
