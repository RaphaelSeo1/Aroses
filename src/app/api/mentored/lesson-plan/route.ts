...
export async function POST(request: Request) {
  ...
  // Check for a cached plan first. Plans generated by an older
  // `generatorVersion` are treated as a miss so prompt upgrades (e.g.
  // adding keyTerms in v2) flow through to existing sessions without a
  // manual reset.
  ...
  if (!body.forceRegenerate) {
    ...
    if (
      ...
      (cached.generatorVersion ?? 0) >= CURRENT_PLAN_VERSION
    ) {
