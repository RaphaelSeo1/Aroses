...
${generationContextSuffix(studyContext, outputLanguage)}
${moduleCount}
Each module must include: numeric "id" (1, 2, 3, … in order), "title", and "lesson_titles" (array of **1 to ${maxLessonTitles}** short strings — concise titles only, no pasted paragraphs). Prefer fewer, denser lesson titles: combine closely related ideas into one title rather than splitting every subheading into its own lesson.
...
- "key_terms" are DISCRETIONARY: keep only genuinely important, study-worthy terms the source defines (not common words); capitalize Latin-script terms properly; an empty key_terms array is valid. Do not invent terms or pad to a count.
