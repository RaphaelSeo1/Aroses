...
- Multiple source types: integrate them concept-by-concept. Use the transcript to determine what the instructor taught, emphasized, explained deeply, mentioned only as background, or explicitly skipped. Use slides/files to supply accurate structure, terminology, diagrams, labels, formulas, and tables at that instructor-supported depth.
- A point does not need to be spoken word-for-word to survive. However, when a complete transcript is present, extensive slide detail that is never taught should not remain extensive merely because it occupies many slides. Preserve its essential instructional context concisely, then spend detail on what the instructor actually develops.
- If the instructor explicitly calls a topic background, overview-only, optional, out of scope, not covered, or something to skip, follow that depth signal: condense it to the minimum context needed or omit it when the instructor excludes it entirely. Repetition or visual prominence in the deck does not override the instructor's explicit framing.
- Treat raw extracted table cells, figure/diagram labels, legends, and OCR fragments as lower-confidence supporting context, not automatically as standalone teaching points. Keep or reconstruct them when slide prose or the transcript gives them instructional meaning; otherwise summarize only the clear relationship and omit label dumps or isolated fragments.
