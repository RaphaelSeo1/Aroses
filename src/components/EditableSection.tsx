...
export function EditableSection({
  ...
  return (
    <div className="group relative rounded-2xl border border-zinc-200/90 bg-zinc-50/40 shadow-sm transition-[border-color,background-color,box-shadow] duration-200 ease-out hover:border-brand-border hover:bg-white/90 hover:shadow-sm dark:border-zinc-800 dark:bg-zinc-950/35 dark:hover:border-brand-border/50 dark:hover:bg-zinc-950/70">
      <div className="flex items-center justify-between gap-3 border-b border-zinc-100/90 px-4 py-2.5 dark:border-zinc-800">
        ...
        {!isEditing ? (
          <button
            type="button"
            onClick={onEdit}
            className="shrink-0 rounded-lg bg-white/90 px-2.5 py-1 text-xs font-semibold text-brand shadow-sm ring-1 ring-brand-border hover:bg-brand-blush hover:text-brand dark:bg-zinc-900 dark:text-brand-soft dark:ring-brand-border/50 dark:hover:bg-brand-blush/10 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100"
          >
            Edit
          </button>
        ) : (
