...
export function TutorSessionStartScreen() {
  ...
  const modeChips: ModeChip[] = useMemo(
    () => [
      {
        id: "exam_prep",
        label: t.tutor.modeExamPrep,
        description: t.tutor.modeExamPrepDesc,
        emoji: "⚡",
      },
      {
        id: "homework_help",
        label: t.tutor.modeHomeworkHelp,
        description: t.tutor.modeHomeworkHelpDesc,
        emoji: "📝",
      },
      {
        id: "concept_review",
        label: t.tutor.modeConceptReview,
        description: t.tutor.modeConceptReviewDesc,
        emoji: "🧠",
      },
      {
        id: "quiz_me",
        label: t.tutor.modeQuizMe,
        description: t.tutor.modeQuizMeDesc,
        emoji: "🎯",
      },
      {
        id: "exploring",
        label: t.tutor.modeExploring,
        description: t.tutor.modeExploringDesc,
        emoji: "🌱",
      },
    ],
    [t.tutor]
      ...
      setFiles((prev) => {
        ...
        for (const f of incoming) {
          ...
          if (next.length >= TUTOR_SESSION_MAX_FILES) {
            setError(
              tf(t.tutor.errMaxFiles, { count: TUTOR_SESSION_MAX_FILES })
            );
            break;
          }
          const nextTotal =
          ...
          if (nextTotal > TUTOR_SESSION_MAX_TOTAL_BYTES) {
            setError(t.tutor.errCombinedSize);
            break;
          }
          if (
            !next.find(
        ...
        if (rejectedUnsupported && next.length === prev.length) {
          setError(
            tf(t.tutor.errUnsupported, { name: rejectedUnsupported })
          );
        }
        return next;
      });
      ...
      try {
        ...
        if (!res.ok) {
          const body = (await res
            .json()
            .catch(() => ({ error: t.tutor.errStartSession }))) as {
            error?: string;
          };
        ...
        setError(e instanceof Error ? e.message : t.tutor.errSomethingWrong);
        setSubmitting(false);
      } finally {
      ...
      <div className="relative">
        ...
        <div className="mt-7">
          ...
          <textarea
            ...
            rows={4}
            placeholder={t.tutor.topicPlaceholder}
