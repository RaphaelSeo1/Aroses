...
import {
  ...
  isValidWorkspaceHostedDomain,
} from "@/lib/school-email-policy";

type Props = {
  /** Safe internal path after OAuth completes (already validated). */
  nextPath: string;
  disabled?: boolean;
  /** Shown on signup vs login — OAuth flow is the same. */
  label?: string;
  /**
...
export function GoogleSignInButton({
  ...
  async function signInWithGoogle() {
    ...
    const envHd = getSchoolGoogleHostedDomain();
    const hd =
      restrictToSchoolWorkspace === false
        ? hostedDomainProp?.trim().toLowerCase()
        : hostedDomainProp?.trim().toLowerCase() || envHd;
    ...
    const { error: oauthError } = await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        ...(hd && isValidWorkspaceHostedDomain(hd)
          ? { queryParams: { hd } }
          : {}),
      },
    });
    ...
    // Browser redirect — loading may never clear; keep spinner until navigation
  }
